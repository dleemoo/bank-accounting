...
RSpec.describe Debit, type: :model do
  ...
  context "given a Transaction with a negative amount" do
    ...
    it "identifies transaction as a Debit" do
      expect(Debit.find_by(id: transaction.id).id).to eq(transaction.id)
    end
  end
  ...
  context "given a Transaction with a positive amount" do
    ...
    it "does not identifies that as a Debit" do
      expect(Debit.find_by(id: transaction.id)).to be_nil
    end
  end
end
